    ...
    <article className="playbook-panel group h-full overflow-hidden transition-[transform,border-color] duration-300 hover:-translate-y-1 hover:border-accent">
      ...
      >
        <span className="relative block aspect-[15/9] w-full overflow-hidden border-b border-border-strong bg-document">
          {course.image ? (
            ...
            <span className="blueprint-dots absolute inset-0 flex items-center justify-between overflow-hidden bg-accent-soft p-7">
              <svg
                className="absolute inset-0 size-full text-accent/30"
                viewBox="0 0 600 360"
                aria-hidden="true"
              >
                <path
                  d="M55 295 C150 210 235 282 300 185 S440 128 545 55"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
                  strokeDasharray="7 7"
              ...
              <span className="relative font-mono text-6xl font-bold tracking-[-0.08em] text-foreground/15">
                {marker}
              </span>
            </span>
          )}
          ...
          <span
            className="absolute inset-0 bg-gradient-to-t from-[#031017]/80 via-transparent to-transparent"
            aria-hidden="true"
          />
          ...
          <span className="absolute bottom-3 left-3 font-mono text-xs font-bold tracking-[0.15em] text-cyan-200">
            {course.code}
          </span>
        </span>
        ...
        <span className="flex flex-1 flex-col p-6 sm:p-7">
          ...
          <span className="mt-4 flex-1 text-sm leading-7 text-muted">
